...
% of the baseline method, in which samples are drawn from the prior,
% reported in Section V of the paper (Table 1; column "Prior (baseline)"). 
% It calls the function run_simulation_baseline with seeds 1:100 and stores 
% the results. The Parallel Computing Toolbox should be used to parallelize 
% the simulations.
...
result_dir = fullfile(pwd, ['results_baseline_', timestamp]);
mkdir(result_dir);
...
parfor seed = 1:N
    try
        result = run_simulation_baseline(seed);
